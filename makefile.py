...
test-sdk-staging:
	...
	$(eval ORIGINAL_LUMI_API_URL := $(shell grep '^LUMI_API_URL=' .env 2>/dev/null | cut -d '=' -f2))
	@if grep -q "^LUMI_API_URL=" .env; then \
		sed -i '' 's|^LUMI_API_URL=.*|LUMI_API_URL=https://staging.lumiteh.cc|' .env; \
	else \
		echo "LUMI_API_URL=https://staging.lumiteh.cc" >> .env; \
	fi
	@echo "Set LUMI_API_URL=$(LUMI_API_URL)"
	@$(SHELL) -c "source .env"
	uv run pytest tests/sdk
	uv run pytest tests/integration/sdk
	@if [ -n "$(ORIGINAL_LUMI_API_URL)" ]; then \
		sed -i '' 's|^LUMI_API_URL=.*|LUMI_API_URL=$(ORIGINAL_LUMI_API_URL)|' .env; \
	else \
		sed -i '' '/^LUMI_API_URL=/d' .env; \
	fi
	@echo "Restored LUMI_API_URL=$(ORIGINAL_LUMI_API_URL)"
	@$(SHELL) -c "source .env"
...
.PHONY: benchmark
benchmark:
	cat benchmarks/benchmark_config.toml | uv run python -m lumiteh_eval.run
...
.PHONY: mcp
mcp:
	uv run python -m lumiteh_mcp.server

.PHONY: mcp-install-claude
mcp-install-claude:
	uv run fastmcp install packages/lumiteh-mcp/src/lumiteh_mcp/server.py -f .env
