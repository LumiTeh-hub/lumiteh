#!/bin/bash
# Quickstart script for Lumi SDK
...
echo -e "\n[INFO] Creating virtual environment at: .venv_lumi_quickstart"
uv venv --python 3.11 ".venv_lumi_quickstart" &> /dev/null

# Step 2: Activate the virtual environment
source .venv_lumi_quickstart/bin/activate
...
uv pip install lumi-sdk &> /dev/null

# Step 4: Run the Python quickstart example
if [ -z "$LUMI_API_KEY" ]; then
  echo "[ERROR] LUMI_API_KEY environment variable is not set."
  echo "You must set this variable to use the Lumi SDK."
  echo "To set it on Unix/Linux/macOS (bash/zsh):"
  echo "  export LUMI_API_KEY=your-key-here"
  echo "To set it on Windows (cmd):"
  echo "  set LUMI_API_KEY=your-key-here"
  echo "To set it on Windows (PowerShell):"
  echo "  $env:LUMI_API_KEY=\"your-key-here\""
  exit 1
fi

# Pull and run the latest quickstart_launcher.py from GitHub
mkdir -p examples
curl -s https://raw.githubusercontent.com/lumi-labs/lumi/refs/heads/main/examples/quickstart_launcher.py -o examples/quickstart_launcher.py
